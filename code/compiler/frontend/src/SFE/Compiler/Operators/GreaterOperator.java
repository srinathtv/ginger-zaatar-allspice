 ...
 * in the program.
 */
public class GreaterOperator extends Operator implements SLPTReduction {
  //~ Methods ----------------------------------------------------------------
  ...
  public BlockStatement toSLPTCircuit(Object obj) {
    ...
    result.addStatement(new AssignmentStatement(
                          lhs,
                          new BinaryOpExpression(new LessOperator(), right, left) //Switch order of operators and use less operator
                        ).toSLPTCircuit(null));
    return result;
  }
  ...
  public Expression resolve(Expression... args) {
    return new LessOperator().resolve(args[1], args[0]);
  }
}
