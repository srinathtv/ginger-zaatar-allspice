...
Prover::Prover(int ph, int b_size, int num_r, int i_size, const char *name) {
  ...
  cout<<"Maximum number of provers is "<<NUM_PROVERS_MAX<<endl;
  cout<<"Actual number of provers is "<<num_procs<<endl;
...
void Prover::handle_terminal_request() {
  ...
  if(is_master) {
    ...
    cout << "p_answer_plainq_par " << p_answer_plainq_par <<endl;
#else
    ...
    // TODO how to fix this for noninteractive?
    printf("p_answer_plainq_par %f\n", p_answer_plainq_par);
#endif
  }
}
...
void Prover::handle_requests() {
  ...
  while(1) {
    ...
    if (phase == PHASE_PROVER_COMMITMENT) {
      ...
      cout << "computation_rclock "<<m_computation.get_rclock_elapsed_time()/num_local_runs<<endl;
