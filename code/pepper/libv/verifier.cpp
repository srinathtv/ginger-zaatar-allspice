...
void Verifier::send_file(char *file_name) {
  ...
  if (!FOLDER_STATE_SHARED) {
    cout<<"Verifier is sending "<<file_name<<endl;
  ...
  if (strstr(file_name, INPUT_FILE_NAME_SUBSTR) != NULL)
...
void Verifier::recv_file(const char *file_name) {
  ...
  snprintf(full_file_name, BUFLEN - 1, "%s/%s", FOLDER_STATE, file_name);
  if (!FOLDER_STATE_SHARED) {
    cout<<"Verifier is receiving "<<file_name<<endl;
...
void Verifier::prepare_answers(int beta) {
  ...
  for (int j=0; j<num_repetitions*num_lin_pcp_queries; j++)
  ...
  snprintf(scratch_str, BUFLEN-1, "input1_b_%d", beta);
...
void Verifier::begin_pepper() {
  ...
  // TODO explain what each measurement does
  ...
  bool result = true;
  cout << "Verifier is running tests." << endl;
  ...
  if (false == result)
    ...
    cout <<endl<<"LOG: The prover passed all the tests"<<endl<<endl;
