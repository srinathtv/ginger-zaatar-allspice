...
static void
computeChiAll(MPZVector& betar, const MPZVector& z, int n, const mpz_t prime, int startAt = 0)
{
  int logn = log2i(n);
  ...
  //   etc.
  mpz_set_ui(betar[0], 1);
  for (int logi = 0; logi < logn; logi++)
  {
    int base = 1 << logi;
    for (int i = base; i < min(2 * base, n); i++)
    {
      modmult(betar[i], betar[i - base], z[logi + startAt], prime);
    }

    one_sub(one_sub_z, z[logi + startAt]);
    for (int i = 0; i < base; i++)
    {
      modmult(betar[i], betar[i], one_sub_z, prime);
    }
  }
  mpz_clear(one_sub_z);
}
...
computeBetaZAll()
{
  computeChiAll(betar, z, outLayer().size(), prime);
}
...
doRoundP(MPZVector& roundPoly, int bitPos, const mpz_t z, const mpz_t r)
{
  ...
  {
    computeChiAll(betar, this->z, nEffectiveBetar, prime, bitPos + 1);
    for (int i = 0;  i < nEffectiveBetar; i++)
    {
