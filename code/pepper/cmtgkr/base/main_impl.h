...
static std::pair<PWSCircuitBuilder*, int>
construct_circuit_batching(int argc, char** argv, char* name)
{
  if(argc < 1)
  {
    std::cerr << "argc < 1. Command line arg should be " << name << " <pws_file> [batch_size] [Na] [Nb]" << endl;
    std::cerr << "   batch_size defaults to 1." << endl;
    std::cerr << "   Na defaults to 32." << endl;
    std::cerr << "   Nb defaults to 1." << endl;
    return std::pair<PWSCircuitBuilder*, int>(NULL, 0);
  }

  PWSCircuitParser* parser = new PWSCircuitParser();
  PWSCircuitBuilder* builder = new PWSCircuitBuilder(*parser);

  MPZVector prime(1);
  Circuit::loadPrime(prime[0]);
  parser->parse(argv[0], prime[0]);
  return std::pair<PWSCircuitBuilder*, int>(builder, 1);
}
...
template<class C> std::pair<C*, int>
construct_circuit(int argc, char** argv, char* name)
{
  if(argc < 1)
  {
    std::cout << "argc < 1. Command line arg should be " << argv[0] << " log(universe size) " << endl;
    return std::pair<C*, int>(NULL, 0);
  }
  ...
  C* circuit = new C(d);
  return std::pair<C*, int>(circuit, 1);
}
