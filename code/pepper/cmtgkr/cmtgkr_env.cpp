...
void
mul_chi(mpz_t rop, const uint64 v, const mpz_t* r, int n, const mpz_t prime)
{
    mpz_t tmp;
    mpz_init(tmp);
...
void
chi(mpz_t rop, const uint64 v, const mpz_t* r, int n, const mpz_t prime)
{
    mpz_t chi;
...
void
computeChiAll(MPZVector& rop, const MPZVector& r, const mpz_t prime)
{
  if (rop.size() == 0)
    return;
  ...
  // Interleave the computations of all the rops.
  // Basically, compute:
  //   1 :  (1 - r1)             r1
  //   2 :  (1 - r1) (1 - r2)    r1 (1 - r2)   (1 - r1) r2   r1 r2
  //   etc.
  mpz_set_ui(rop[0], 1);
  for (size_t logi = 0; logi < r.size(); logi++)
  {
    int base = 1 << logi;
    for (int i = base; i < min(2 * base, n); i++)
    {
      modmult(rop[i], rop[i - base], r[logi], prime);
    }

    one_sub(one_sub_r, r[logi]);
    for (int i = 0; i < min(base, n); i++)
    {
      modmult(rop[i], rop[i], one_sub_r, prime);
    }
  }
  mpz_clear(one_sub_r);
}
...
void
evaluate_beta_z_ULL(mpz_t rop, int mi, const mpz_t* z, const uint64 k, const mpz_t prime)
{
...
bary_precompute_weights(MPZVector& weights, const mpz_t r, const mpz_t prime)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    modmult(lx, lx, tmp, prime);
  }

  for (int i = 0; i < n; i++)
    modmult(weights[i], weights[i], lx, prime);
