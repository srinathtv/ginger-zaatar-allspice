...
class PWSCircuitParser
{
  ...
  void parse(const std::string& pwsFileName, const mpz_t prime);
  ...
  void parseLessThanFloat(Tokenizer& pws, const mpz_t prime);
  void parseNotEqual(Tokenizer& pws);
  void parseDivide(Tokenizer& pws, const mpz_t prime);
