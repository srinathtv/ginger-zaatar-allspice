...
static bool
recursive_mkdir(const string& dir, mode_t mode = DIRECTORY_PERMISSION)
{
  struct stat sb;
...
CircuitData(size_t memBudget, const std::vector<size_t>& sizes, const string& suffix)
  : layers(), layerSizes(sizes), circuitDir(), budget(memBudget), usage(0)
{
  stringstream ss;
  bool success;
...
{
  assert(inRange<int>(layerIdx, 0, layerSizes.size()));
  typedef typename deque<LayerData<T> >::iterator LayerDataIt;
  for (LayerDataIt it = layers.begin(); it != layers.end(); ++it)
  {
...
save() const
{
  ...
  for (LayerDataCIt it = layers.begin(); it != layers.end(); ++it)
...
load(int layerIdx)
{
  ...
  {
    LayerData<T>& layer = layers.front();
  ...
  layers.push_back(LayerData<T>(layerIdx));
  LayerData<T>& newLayer = layers.back();
  newLayer.tryLoad(circuitDir, layerSizes[layerIdx]);
...
shouldEvict(bool evictToAdd) const
{
  ...
  return (minNumLayers < layers.size()) && (usage > budget);
}
