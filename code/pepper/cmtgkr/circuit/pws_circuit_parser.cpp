...
#define ASSERT_VALID_RETURN(maybe)  if (!maybe.isValid()) return;
...
PWSCircuitParser::
PWSCircuitParser()
  : varMap(), inOutVarsMap(), outConstantDesc(), constants(), constMap(), circuitDesc(), token()
{ }
...
void PWSCircuitParser::
parse(const string& pwsFileName, const mpz_t prime)
{
  clear();
  ...
  while (pws.hasNext())
  {
    ...
    */
    ...
    if (token == "P")
      ...
      parseLessThanFloat(pws, prime);
    else if (token == "!=")
      parseNotEqual(pws);
    else if (token == "/")
      parseDivide(pws, prime);
...
getCircuitVariable(const Primitive& p)
{
  Maybe<CVar*> out;
  if (p.isVariable())
    return out.validate(getVarMap(p)[p.idx]);
  return out;
}
...
parseVar(const string& token)
{
  ...
  {
    case 'I':
      varType.validate(CVar::INPUT);
      break;
    case 'O':
      varType.validate(CVar::OUTPUT);
      break;
    case 'V':
      varType.validate(CVar::TEMP);
      break;
    case 'M':
      varType.validate(CVar::MAGIC);
      break;
    default:
      varType.invalidate();
      break;
...
void PWSCircuitParser::
parseLessThanFloat(Tokenizer& pws, const mpz_t prime)
{
  vector<int> opGuard;
  getOpGuard(opGuard);
...
void PWSCircuitParser::
parseDivide(Tokenizer& pws, const mpz_t prime)
{
  Maybe<Primitive> p = parsePrimitive(pws);
